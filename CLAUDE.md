...
- 現状どの config でも `[jev] enabled = false`（既定）のままであり、TypeSafe（Jev、System One）への送信は発生していない。`[jev] enabled = true` にすると、**第1層 advisory のエスカレーションルール（`server/data/urtect/rules.json` の `warranty-failure` / `contract-billing`）にマッチしたターンに限り、顧客発話が TypeSafe へ送信される**（Issue #58）。送信されるのは今ターンの発話だけでなく、直近の顧客発話（最大 6 件・合計 2,000 字まで。発話は切り詰めず、超過分は古い側から丸ごと落とす）も時系列昇順で前置した文字列である（assistant 発話は送らない。組み立ては `server/src/api.rs::build_jev_state`）。送信された発話に対する `has_enough_info`（0〜1）が、聞き返し（Clarify）vs 即エスカレーション（EscalationReply）の判定に使われる。閾値は `[jev] enough_info_threshold`（既定 0.5）。Jev の障害・タイムアウト・応答欠落は従来の `missing` ベース判定（`decide_reply_action`）へ fail-back する（応答を止めない）。判定に Jev を使うのはこの経路だけで、第1層 mandatory・第2層・第3層・MCP `evaluate_answerability` 経路は従来どおり Jev を呼ばない。正本は `docs/superpowers/specs/2026-09-21-jev-shadow-design.md` §7。**Issue #56 設計の「fire-and-forget の shadow ログ記録（全質問の結果を `tracing::info!` で 1 行残す）」自体は依然として未実装。** 質問定義は `server/data/urtect/jev-questions.json`。
