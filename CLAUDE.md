...
- `[jev] enabled = true` にすると、**顧客発話が TypeSafe（Jev、System One）へも送信される**（Issue #56。判定には未接続の shadow 実行）。現状どの config でも `[jev] enabled = false`（既定）のままであり、送信は発生していない。**さらに、呼び出しの起動点（fire-and-forget 呼び出し）自体が Issue #57 で未実装のため、現時点では `enabled = true` にしても呼び出しは一切発生しない。** 質問定義は `server/data/urtect/jev-questions.json`。
