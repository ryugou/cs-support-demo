...
- **`server/data/urtect/rules.json` / `server/data/urtect/signal-lexicon.json` はどちらも image 内ファイルが正本**（service と `ingest-rules` job は同一イメージ。Product ノードとは扱いが異なる点に注意）。main マージ時は CI（`auto-ingest` job、`.github/workflows/deploy.yml`）が `server/data/**` の変更を検知し、ビルド → デプロイ → `ingest-rules` 実行までを自動で行う。CI 以外の経路で image を反映しない。`server/data/**` を変更していないのに `rules.json` の再投入だけが必要なとき（＝`auto-ingest` の paths filter が発火しないとき）に限り、`gcloud run jobs execute ingest-rules --project sivira-cs-support --region asia-northeast1 --wait` を単発で実行してよい（`jobs execute` は image を更新しないため、手動デプロイ禁止の対象外）。**`signal-lexicon.json` は vegapunk へ投入する対象ではなく、サービスが起動時にイメージ内ファイルから読むだけなので `ingest-rules` では一切反映されない**（反映漏れは signal が本番で永久に立たないサイレント never-match になる。`ingest_rules.rs` は `--rules-file` のみを受け取り lexicon ファイルには一切触れない）。**`ingest_rules` は rules.json の未知フィールド名（綴りミス。`deny_unknown_fields`）・`binding` の省略（**`binding` は必須で既定値は無い**。`escalation_rules` と `prohibited_domains` の両方）・未知の `binding` 値（`"manadatory"` / `"Mandatory"` 等。完全一致のみ受理）・未知の `hearing` 値・mandatory への `hearing` 宣言を、vegapunk へ接続する前（＝何も書く前）にエラーで拒否する**（`ingest-rules` job が失敗する。serde の既定では綴りミスが黙って無視され、本番で Jev が永久に呼ばれない・禁止領域が signal で止まらない、というサイレントな never-match になるため。とくに `binding` の綴りミス・書き忘れは、実行時のローダが未知の値・属性なしを advisory に倒す（fail-back。未知値は warn が出る）ため、mandatory の即時エスカレーション保証が失われ `human-handoff` のような脱出手段が Jev のヒアリングに吸収されうる。入力形式を狭めるのは意図した fail-closed で、現行の `rules.json` / `rules.sample.json` は全件明示している）。
- **製品マスタの正本は vegapunk の Product ノード**（Issue #6: `KNOWN_MODELS` 定数は廃止済み）。`server/data/urtect/products.json` はコードではなく、`ingest_products` CLI に渡す seed 投入の入力記録である。
  ...
  - **`ingest-products` は CI の `auto-ingest` job（`server/data/**` 変更検知）の対象に含まれない。** `server/data/urtect/products.json` を変更して main へマージしても、デプロイは自動反映しない。`/api/reply` の取扱製品スコープ（Issue #28）は vegapunk の Product ノード一覧を正本にするため、**マージ・デプロイ完了後に必ず手動で `gcloud run jobs execute ingest-products --project sivira-cs-support --region asia-northeast1 --wait` を実行すること。** 実行するまでの間、`products.json` に新規追加した型番は「当社では取り扱いがございません」という誤った取扱外応答の対象になる（既存の取扱製品には影響しない）。
  - 全体リセット後、`ingest_products` は `ingest_urtect` より必ず先に実行すること。`ingest_urtect` は起動時に vegapunk の Product ノード一覧を取得し、0 件なら「製品マスタが空。先に `ingest_products` を実行せよ」という fail closed で止まる。`ingest_alarmcom`（answers.alarm.com）は Product ノードに依存しないため、`ingest_products` との順序制約は無い。`ingest_urtect`（Google Sites）と `ingest_alarmcom` の間にも順序依存は無く、どちらを先に走らせてもよい。
  ...
  - **第 2 のマニュアルソース `ingest_alarmcom`（Issue #8、v2）**: answers.alarm.com（MindTouch KB）の `/Customer` + `/Partner` 配下の全 URL を sitemap.xml 駆動でクロールする（製品マスタによる絞り込みは行わない。v1 にあったファミリーハブ一致判定・製品マスタ 0 件時 fail closed は v2 で全廃した）。英語版のみ 1 fetch/記事で取得し、`?mt-language=JA` の機械翻訳は使わず Gemini（`translate::translate_and_extract`）で自前翻訳する。本文は `#elm-main-content` から、breadcrumb は `.mt-breadcrumbs` から抽出する。alarm.com は製品非依存のため新規に `DESCRIBES` 辺は張らない（v1 時代の残存 `DESCRIBES` は stale edge として検出時に除去する）。親ページが未 ingest の子記事はスキップせず、`PARENT_OF` を張らずに document 直下の root section として投入する。fetch 失敗・本文空・breadcrumb 欠落・翻訳失敗などサイト構造由来の skip が、最低 20 件処理した時点以降で処理件数の 20% を超えたら bail する（サイト構造・抽出セレクタの変化を疑う）。既存 section が今回のクロール対象一覧から消えていた場合も、backend に delete が無いため fail closed で bail する。robots.txt の Crawl-delay=5 秒を守るため全リクエスト（sitemap 含む）を 5 秒以上空けて逐次実行し、**実行時間は対象記事数（≒英語 1 リクエスト × 記事数 × 5 秒 + Gemini 翻訳時間）に比例する**。Cloud Run job `ingest-alarmcom` は CI の image 更新対象に含まれる（`.github/workflows/deploy.yml` の `RUN_JOBS`）。**job が未作成のまま main にマージすると `Update Cloud Run job images` ステップが `NOT_FOUND` で失敗し、デプロイ経路全体（service 更新を含む）が止まる。**
...
- env（fail-closed 境界で2群に分けて扱うこと）:
  - **未設定だと起動に失敗する**: `CS_SUPPORT_PUBLIC_DOMAIN`、`CS_SUPPORT_GOOGLE_OAUTH_CLIENT_ID`、`CS_SUPPORT_GOOGLE_OAUTH_CLIENT_SECRET`、`CS_SUPPORT_LLM_API_KEY`（`config.cloudrun.toml` が `[llm] enabled = true` のため。鍵を解決できないと `server/src/llm.rs` の `AnthropicClient::from_config` で起動時 fail closed）、`CS_SUPPORT_ANSWER_API_KEY`（`config.cloudrun.toml` が `[api] enabled = true` のため。`/{project_id}/api/reply` の Bearer 認証キーを解決できないと `server/src/main.rs` の起動時チェックで fail closed。これは `cs-support-mcp` 本体側の話で、`cs-support-line`（LINE アダプタ）側の必須 env は別立て。下記「LINE アダプタ」節を参照）、`TYPESAFE_API_KEY`（`config.cloudrun.toml` が `[jev] enabled = true` のため必須。鍵を解決できないと `server/src/jev.rs` の `JevClient::from_config` で起動時 fail closed。他の必須 env と異なり `CS_SUPPORT_` prefix が付かない点に注意。`[jev] enabled = false` の構成（他の config）では不要）
...
- **本番 Cloud Run 用の設定（`config.cloudrun.toml`）は `[jev] enabled = true` であり、この設定が反映された revision では顧客発話が第三者（TypeSafe）へ送信される**（他の config は既定の `[jev] enabled = false` のままで送信しない。運用上の注意点として認識しておくこと。`[llm] enabled = true` による Anthropic への送信と同じ扱い）。反映は main マージ後の CI デプロイで行われる。送信されるのは、**`server/data/urtect/rules.json` のうちヒアリング契約 `product_and_symptom`（`"hearing"` フィールド）を宣言した第1層ルール、すなわち `warranty-failure`（製品の故障）にマッチしたターンに限る**（Issue #58）。同じ advisory の `contract-billing`（契約・請求）は型番も症状も関係ない問い合わせで、`has_enough_info`（「対象の製品と具体的な症状の両方が分かるか」）で測ると無関係なヒアリングへ流れるため、宣言を持たず**対象外**（Jev を呼ばず、顧客発話も送信しない）。判別は binding や rule_id ではなくルール自身の宣言で行い、mandatory ルールは宣言があっても無効。宣言は vegapunk の `EscalationRule` ノードの `hearing` 属性として `ingest-rules` が投入する（反映前は「宣言なし」として従来判定へ fail-back）。送信されるのは今ターンの発話だけでなく、直近の顧客発話（最大 6 件・合計 2,000 字まで。発話は切り詰めず、超過分は古い側から丸ごと落とす）も時系列昇順で前置した文字列である（assistant 発話は送らない。組み立ては `server/src/api.rs::build_jev_state`）。送信された発話に対する `has_enough_info`（0〜1）が、聞き返し（Clarify）vs 即エスカレーション（EscalationReply）の判定に使われる。閾値は `[jev] enough_info_threshold`（既定 0.5）。Jev の障害・タイムアウト・応答欠落は従来の `missing` ベース判定（`decide_reply_action`）へ fail-back する（応答を止めない）。**切り戻しは `server/config.cloudrun.toml` のこの行を `false` にして再デプロイするだけ**で、Jev を呼ばなくなり判定は従来の `missing` ベース（`decide_reply_action`）へ完全に戻る。判定に Jev を使うのはこの経路だけで、宣言の無い第1層ルール（`contract-billing` を含む）・第1層 mandatory・第2層・第3層・MCP `evaluate_answerability` 経路は従来どおり Jev を呼ばない。正本は `docs/superpowers/specs/2026-09-21-jev-shadow-design.md` §7。**Issue #56 設計の「fire-and-forget の shadow ログ記録（全質問の結果を `tracing::info!` で 1 行残す）」自体は依然として未実装。** 質問定義は `server/data/urtect/jev-questions.json`。
...
- `ingest-homesec` job: `server/data/homesec/materials.json`（image 内が正本）を vegapunk schema `homesec` へ冪等投入。**CI の `auto-ingest` は `ingest-rules` しか実行しないため、materials.json を変更したらマージ・デプロイ後に `gcloud run jobs execute ingest-homesec --project sivira-cs-support --region asia-northeast1 --wait` を手動実行すること**（未実行だと旧材料のまま応答する）。**`schema/homesec.yml` の属性を変更した場合も同じコマンドの実行が必要**（`ingest_homesec.rs` の `create_or_update_schema` が本番 vegapunk の schema 登録を担っており、`materials.json` を変更していなくても、未実行のままだと新しい属性が未宣言のまま support_case へ書き込まれうる）
