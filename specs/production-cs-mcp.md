...
外部の型付き判断器 Jev（TypeSafe System One、`server/src/jev.rs`）を、**第1層 advisory のエスカレーションターンに限り**、聞き返し（Clarify）か即エスカレーション（EscalationReply）かの分岐に使う（Issue #58）。Jev の `has_enough_info`（0〜1。質問定義は `server/data/urtect/jev-questions.json`）が「顧客が話した情報の十分性」を測る。マニュアル材料のカバレッジ不足で決まる `missing` は顧客情報の十分性を表さないため、この経路に限り置き換えた。

- **適用対象**: `/{project_id}/api/reply` の、第1層 advisory ルールにマッチしたターンのみ（`layer == 1` かつ `rule_binding == Some(Binding::Advisory)`、抽出が lexicon fallback に落ちておらず、case が確定済みでないこと）。
- **第三者（TypeSafe）への送信範囲**: 送信されるのは今ターンの顧客発話に加えて、過去の顧客発話（最大 6 件・合計 2,000 字まで、時系列昇順）。発話は途中で切り詰めず、超過分は古い側から丸ごと落とす。assistant 発話は送信しない。送信が発生するのは対象ターン（上記の第1層 advisory のエスカレーションターン）に限る。現状どの config も `[jev] enabled = false` のため、本番ではまだ送信は発生していない。
- **従来どおり Jev を使わない**: 第1層 mandatory・第2層・第3層・MCP `evaluate_answerability` 経路。mandatory の即時エスカレーション契約は不変。
...
- **途中で切れた下書きは `customer_reply_draft_truncated = true` で申告する**（生成上限に達したか＝上流の `stop_reason` が `max_tokens` か）。**切断を「短くなっただけ」と扱わないこと。** 理由は 2 つあり、どちらも切れた文面が完成品に見えることに由来する
  - **出口ゲートの統制が構造的に迂回されうる。** `egress_gate` は NG 語の**部分文字列一致**で判定するため、「絶対に治ります」が「絶対に治りま」で切れると**ゲートを通過する**。主張は読者に伝わるのに統制だけが外れる。上記「出口ゲートを必ず通す」は**通したこと**を保証するが、**切断後も有効であること**は保証しない
  - **末尾の但し書きが落ちうる。** 切れ目が句点の直後だと文面は完成して見えるが、日本語のビジネス文は結び・注意書き（「電源を切ってから作業してください」等）が末尾に来るため、**安全上の但し書きだけが欠けた案内**になりうる
  - **運用契約**: `decision.decision` が `allowed` のときに限り `hits` の原文と突き合わせて補ってよい。**`escalate` のときは補わない** —— サーバが意図的に渡さなかった本文で補完することは、回答してはいけないと判定した場面で答えることに等しく、判定そのものを無効化する。切れた下書きは破棄し、取り次ぐ旨だけを書く
  - 上限を上げる・切れたら再生成する等の自動回復は**現時点で採っていない**。デモでは切断自体が稀（escalate の下書きは短く指示している）で、シグナルを出して人が判断する形で足りると判断した
...
- **LINE アダプタは判断ゼロ。** 署名検証・応答生成 API への 1 コール・LINE への返信・ユーザ単位の会話履歴保持（プロセス内メモリ）のみを行い、判定・生成ロジックを一切持たない。会話履歴は原則、生成プロンプトへの注入にのみ使い、判定（signal 抽出・第1〜3層の escalation 判定・signal 累積）には使わない（既存の case 機構が判定側のターン間文脈を担う）。**例外**: 第1層 advisory の聞き返し判定（Jev の `has_enough_info`）に限り、顧客発話の履歴が判定入力になる（`[jev] enabled = true` のときのみ動く。適用範囲は「追記: Jev（TypeSafe System One）の適用範囲（Issue #56 / #58、2026-09-21）」節、正本は `docs/superpowers/specs/2026-09-21-jev-shadow-design.md` §7）。
