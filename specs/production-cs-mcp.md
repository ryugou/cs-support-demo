...
- **Allowed では判定が根拠に採った材料のみを渡す。** manual 由来は `evidence_section_keys` の section だけ（`hits` 全件ではない）。KR 由来は `evidence_section_keys` が空になるため `kr.answer`（承認済みの正本回答）を渡す。引けない場合は材料ゼロのままとし、**でっち上げた材料は渡さない**
