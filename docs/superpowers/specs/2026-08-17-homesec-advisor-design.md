...
- サポートモードのターンは、既存 CS パイプラインを CS Demo と同一構成(schema = urtect)のまま関数として呼ぶ。マニュアル検索・三層 fail-closed 判定・KR 照合・signal 蓄積・聞き返し・エスカレーション・case/ターンの記録まで、既存 CS の 1 会話と同じ動作
- CS 側コードの変更は「HTTP ハンドラから呼び出し可能な関数への切り出し」等の挙動不変の最小加算のみ。既存(urtect)テストが無変更で PASS することを機械的条件とする
