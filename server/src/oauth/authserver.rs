...
impl AuthServerState {
    ...
    ///   再提示された場合」だけに限られる。
    ///   - **2026-08 注記: この窓は署名鍵の永続化によって初めて実在するようになった。**
    ///     それ以前は再起動で鍵が変わり、`open::<Blob>(&code)` が `consume_jti` へ到達する
    ///     前に落ちていたため、この経路は構造的に成立しなかった。リスク評価は変わらない
    ///     （`CODE_TTL_SECS` = 60 秒 + 下記 PKCE 束縛）が、「起きない」ではなく
    ///     「起きても無害」に根拠が移っている。
